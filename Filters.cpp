...
LowPassButter::LowPassButter(Input<double> input, double cutoff, unsigned int order, ModuleRegistry& registry) : Module(registry), in_input(input), out_output(0.0) {
    n = order;
...
HighPassButter::HighPassButter(Input<double> input, double cutoff, unsigned int order, ModuleRegistry& registry) : Module(registry), in_input(input), out_output(0.0) {
    n = order;
    double a = tan(M_PI * cutoff / SOUND_FREQUENCY);
    double a2 = a * a;
...
BandPassButter::BandPassButter(Input<double> input, double lowerCutoff, double upperCutoff, unsigned int order, ModuleRegistry& registry) : Module(registry), in_input(input), out_output(0.0) {
    n = order;
...
BandStopButter::BandStopButter(Input<double> input, double lowerCutoff, double upperCutoff, unsigned int order, ModuleRegistry& registry) : Module(registry), in_input(input), out_output(0.0) {
    n = order;
    double a = cos(M_PI * (upperCutoff + lowerCutoff) / SOUND_FREQUENCY) / cos(M_PI * (upperCutoff - lowerCutoff) / SOUND_FREQUENCY);
    double a2 = a * a;
    double b = tan(M_PI * (upperCutoff - lowerCutoff) / SOUND_FREQUENCY);
